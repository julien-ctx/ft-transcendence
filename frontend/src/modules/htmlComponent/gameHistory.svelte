...
{#each gameHistory as game}
	{#if game.user[0] && game.user[1]}	
		<div class="flex justify-between items-center max-w-lg shadow-md p-5 rounded">
			<div>
				<Avatar size="lg" src={game.img_link_user1} class="object-cover bg-transparent" rounded/>
				<p class="capitalize font-medium text-center">{game.login_user1}</p>
			</div>
			...
			<div>
				<Avatar size="lg" src={game.img_link_user2} class="object-cover bg-transparent" rounded/>
				<p class="capitalize font-medium text-center">{game.login_user2}</p>
			</div>
		</div>
	{/if}
{/each}
